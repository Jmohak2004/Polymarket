...
export default function HomePage() {
  ...
  return (
    <div>
      <div className="mb-10 border-2 border-neutral-950 bg-[#fffef8] p-6 sm:p-8" style={{ boxShadow: "6px 6px 0 0 #0a0a0a" }}>
      ...
      <div className="mb-8 grid grid-cols-3 gap-3 sm:gap-4">
        {[
          { label: "Total", value: markets.length },
          { label: "Open", value: markets.filter((m) => m.status === 0).length },
          { label: "Resolved", value: markets.filter((m) => m.status === 2).length },
        ].map((stat) => (
          <div
            key={stat.label}
      ...
      {!loading && !error && markets.length === 0 && (
        ...
        >
          ...
          <p className="mb-3 font-bold text-neutral-800">No markets yet.</p>
          <Link href="/create" className="text-sm font-bold text-neutral-950 underline decoration-2 underline-offset-4">
